...
from connect.eaas.extension_runner.exceptions import EaaSError
...
def test_get_container_id(mocker):
    ...
    mocked = mocker.patch(
        'connect.eaas.extension_runner.helpers.subprocess.run',
        return_value=result,
    )
...
def test_get_container_id_cgroups2_fallback(mocker):
    ...
    mocker.patch('connect.eaas.extension_runner.helpers.uuid4', return_value='test_uuid')
    result_cpuset = mocker.MagicMock()
    result_cpuset.returnvalue = 0
    result_cpuset.stdout = (
        '/\n'.encode('utf-8')
...
def test_get_container_id_ko(mocker):
    ...
    mocker.patch('connect.eaas.extension_runner.helpers.subprocess.run', return_value=result)
...
def test_get_environment_with_defaults(mocker):
    ...
    mocker.patch(
        'connect.eaas.extension_runner.helpers.get_container_id',
...
def test_get_extension_class(mocker):
    ...
    mocker.patch(
        'connect.eaas.extension_runner.helpers.iter_entry_points',
        return_value=iter([
...
def test_get_version(mocker):
    ...
    mocked.version = '22.0'
    mocker.patch(
        'connect.eaas.extension_runner.helpers.get_distribution',
        return_value=mocked,
    )
