...
from connect.eaas.extension_runner.constants import RESULT_SENDER_MAX_RETRIES
...
from connect.eaas.extension_runner.exceptions import (
    CommunicationError,
    MaintenanceError,
    StopBackoffError,
)
...
async def test_extension_settings(mocker, ws_server, unused_port, settings_payload):
    mocker.patch(
        'connect.eaas.extension_runner.config.get_environment',
        return_value={
    ...
    mocker.patch('connect.eaas.extension_runner.handler.get_extension_type', return_value='sync')
    mocker.patch('connect.eaas.extension_runner.worker.get_version', return_value='24.1')
...
async def test_pr_task(mocker, ws_server, unused_port, httpx_mock, settings_payload):
    ...
    mocked_time = mocker.patch('connect.eaas.extension_runner.managers.background.time')
...
async def test_scheduled_task(mocker, ws_server, unused_port, httpx_mock, settings_payload):
    ...
    mocked_time = mocker.patch('connect.eaas.extension_runner.managers.scheduled.time')
    mocked_time.sleep = time.sleep
    mocked_time.monotonic.side_effect = (1.0, 2.0)
    handler = WSHandler(
        '/public/v1/devops/ws/ENV-000-0001/INS-000-0002?running_tasks=0&running_scheduled_tasks=0',
...
async def test_connection_closed_error(mocker, ws_server, unused_port, caplog):
    mocker.patch('connect.eaas.extension_runner.worker.MAX_RETRY_TIME_GENERIC_SECONDS', 1)
    mocker.patch('connect.eaas.extension_runner.worker.MAX_RETRY_DELAY_TIME_SECONDS', 1)
    mocker.patch('connect.eaas.extension_runner.worker.DELAY_ON_CONNECT_EXCEPTION_SECONDS', 0.1)
...
async def test_connection_maintenance(mocker, ws_server, unused_port, caplog):
    mocker.patch('connect.eaas.extension_runner.worker.DELAY_ON_CONNECT_EXCEPTION_SECONDS', 0.1)
    mocker.patch('connect.eaas.extension_runner.worker.MAX_RETRY_TIME_MAINTENANCE_SECONDS', 1)
    mocker.patch('connect.eaas.extension_runner.worker.MAX_RETRY_DELAY_TIME_SECONDS', 1)
    mocker.patch('connect.eaas.extension_runner.handler.get_extension_class')
    mocker.patch('connect.eaas.extension_runner.handler.get_extension_type')
    mocker.patch(
...
async def test_ensure_connection_maintenance(mocker, caplog):
    ...
    mocker.patch('connect.eaas.extension_runner.worker.MAX_RETRY_TIME_MAINTENANCE_SECONDS', 1)
    mocker.patch(
        'connect.eaas.extension_runner.worker.websockets.connect',
...
async def test_ensure_connection_exit_backoff(mocker, caplog):
    mocker.patch('connect.eaas.extension_runner.handler.get_extension_class')
    mocker.patch('connect.eaas.extension_runner.handler.get_extension_type')
    mocker.patch('connect.eaas.extension_runner.worker.MAX_RETRY_TIME_GENERIC_SECONDS', 600)
    mocker.patch('connect.eaas.extension_runner.worker.MAX_RETRY_DELAY_TIME_SECONDS', 1)
    mocker.patch(
        'connect.eaas.extension_runner.worker.websockets.connect',
        side_effect=RuntimeError('generic error'),
...
async def test_ensure_connection_exit_max_attemps(mocker, caplog):
    mocker.patch('connect.eaas.extension_runner.handler.get_extension_class')
    mocker.patch('connect.eaas.extension_runner.handler.get_extension_type')
    mocker.patch('connect.eaas.extension_runner.worker.MAX_RETRY_TIME_GENERIC_SECONDS', 10)
    mocker.patch('connect.eaas.extension_runner.worker.MAX_RETRY_DELAY_TIME_SECONDS', 1)
    mocker.patch(
        'connect.eaas.extension_runner.worker.websockets.connect',
        side_effect=RuntimeError('generic error'),
...
async def test_shutdown_pending_task_timeout(mocker, ws_server, unused_port, settings_payload):
    ...
    mocker.patch('connect.eaas.extension_runner.handler.get_extension_type', return_value='sync')
    mocker.patch('connect.eaas.extension_runner.worker.RESULT_SENDER_WAIT_GRACE_SECONDS', .1)
